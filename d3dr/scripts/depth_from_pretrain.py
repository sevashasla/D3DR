...
Run with:
    python d3dr/scripts/zoe_depth_completion.py
...
from d3dr.utils.camera_utils import euclidean_to_z_depth
from d3dr.utils.utils import (
    SCALE_FACTOR,
...
class DepthsFromPretrained:
    ...
    def main(self):
        if os.path.exists(os.path.join(self.data_dir, "transforms.json")):
            ...
            image_paths = [
                self.data_dir / Path(frame["file_path"])
            ...
            image_paths = get_filename_list(
                self.data_dir / Path(self.img_dir_name)
            )
            run_monocular_depths(
                images=image_paths,
                save_path=self.save_path,
...
def depth_align(
    ...
    """
    assert depths.shape[:] == est_depths.shape[:], (
    ...
    for batch_index in range(0, num_frames, batch_size):
        ...
        with torch.no_grad():
            ...
            for frame_index in range(len(batch_frames)):
                frame = images[batch_index : batch_index + batch_size][
                    frame_index
                ]
...
def depth_from_pretrain(
        ...
        for batch_index in range(0, len(meta["frames"]), batch_size):
            ...
            with torch.no_grad():
                ...
                for frame_index in range(len(batch_frames)):
                    ...
                    image = image_path_to_tensor(
                        input_folder / frame["file_path"]
                    ).to(device)
                ...
                for i in track(
                    ...
                    image = image_tensors[i]
                    image = (
                        torch.permute(image, (2, 0, 1)).unsqueeze(0).to(device)
                    )
                    if image.shape[1] == 4:
                        image = image[:, :3, :, :]
                    if pretrain_model == "zoe":
                        mono_depth_tensor = (
                            zoe.infer(image).squeeze().unsqueeze(-1)
                        )
                    else:
                        raise NotImplementedError
            ...
            with torch.no_grad():
                ...
                for frame_index in range(len(batch_frames)):
                    ...
                    if return_mode == "mono-aligned":
                        ...
                        depth = depth_path_to_tensor(
                            (
                                frame.parent.parent / "depth" / frame.name
                            ).with_suffix(".png"),
                            return_color=False,
                            scale_factor=SCALE_FACTOR,
                        )
    ...
    if create_new_transforms:
        for idx in range(num_frames):
            ...
            file_save_path = Path(save_path.stem) / Path(file_name).with_suffix(
                ".png"
            )
            meta["frames"][idx].update(
                {"mono_depth_file_path": str(file_save_path)}
            )
