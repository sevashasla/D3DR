...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="path to the depth/normal/etc for controlnet",
)
...
def preprocess_depth_image(image_path):
    ...
    controlnet_image = center_crop_to_square(
        controlnet_image
    )  # depth in my case
...
def read_image_mask(
    ...
    if args.add_mean_init:
        mask = torch.from_numpy(init_mask[None, None, ...]).to(
            device=torch_device, dtype=guidance.precision_t
        )
        image_torch = (
            image_torch * (1.0 - mask)
            + masked_mean(image_torch, mask, dim=(2, 3))[..., None, None] * mask
        )
